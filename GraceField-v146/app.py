...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.46', page_icon='⚖️', layout='wide', initial_sidebar_state='expanded')
...
st.title('Extrator DIRF + Motor Previdenciário V1.2.46')
...
@st.dialog('🔎 Evidência documental — CNIS')
def _modal_evidencia_cnis(cnpj, ev, imagem):
    ...
    if imagem:
        st.image(imagem, use_container_width=True, caption=f'CNIS — página {ev.get("pagina") or "—"}')
    else:
        st.warning('Não foi possível gerar o recorte visual desta evidência.')
        ...
        for cnpj in cnpjs:
            ...
            with st.container(border=True):
                ...
                if ev_cnis:
                    ...
                    st.markdown(f'**Fonte documental da identificação:** CNIS · **{cnis_natureza}**')
                    ev0 = ev_cnis[0]
                    img = None
