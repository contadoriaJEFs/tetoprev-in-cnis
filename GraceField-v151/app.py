...
st.set_page_config(page_title='Extrator DIRF + Motor Previdenciário V1.2.50', page_icon='⚖️', layout='wide', initial_sidebar_state='expanded')
...
def _confirmar_classificacao_usuario(cnpj, widget_key, meta):
    """Registra uma decisão manual somente após o clique de confirmação.

    O selectbox é apenas uma edição pendente. A classificação efetivamente
    utilizada pelo motor permanece a classificação automática do CNIS até que
    o usuário confirme uma alteração.
    """
    val = st.session_state.get(widget_key, meta.get('classificacao_final', meta.get('classificacao_sugerida', 'nao_definido')))
    ...
    st.session_state.classification_meta[cnpj] = {
        **meta,
        ...
        'confirmacao_manual_explicita': True,
...
st.title('Extrator DIRF + Motor Previdenciário V1.2.50')
...
with st.expander('📑 Abrir relatório por página', expanded=False):
    ...
    for r in paginas:
        ...
        # Streamlit não permite expander dentro de outro expander.
        # Usamos container com borda para cada página, preservando o relatório recolhível no nível superior.
        with st.container(border=True):
            st.markdown(f"**Página {pagina} — {r.get('nome_declarante_dirf') or 'Declarante não identificado'}**")
            info_rows = {
...
with tabs[2]:
    ...
    if not cnpjs:
        ...
        for cnpj in cnpjs:
            ...
            # A classificação automática calculada nesta leitura é a que alimenta
            # o motor, salvo decisão manual explicitamente confirmada.
            # Isso também corrige arquivos .tetoprev antigos que tenham uma
            # classificação automática/legada diferente da evidência CNIS atual.
            meta_manual = st.session_state.classification_meta.get(cnpj, {}) or {}
            manual_confirmada = bool(meta_manual.get('confirmacao_manual_explicita'))
            if not manual_confirmada:
                automatico = meta.get('classificacao_final', meta.get('classificacao_sugerida', 'nao_definido'))
                st.session_state.assignments[cnpj] = automatico
                # Guarda o estado automático sem caracterizá-lo como decisão do usuário.
                st.session_state.classification_meta[cnpj] = {
                    **meta,
                    'classificacao_final': automatico,
                    'origem_classificacao': meta.get('origem_classificacao', 'automatico'),
                    'confirmacao_manual_explicita': False,
                }
                meta_manual = st.session_state.classification_meta[cnpj]

            current = st.session_state.assignments.get(cnpj, meta.get('classificacao_final', meta['classificacao_sugerida']))
            if current not in ('nao_definido', '11', '20', 'progressiva'):
                current = meta.get('classificacao_final', meta['classificacao_sugerida'])
            ...
            with st.container(border=True):
                ...
                if ev_cnis:
                    ...
                    with b1:
                        if st.button('🔎 Ver CNIS', key='ver_cnis_' + re.sub(r'\D+', '', cnpj), use_container_width=True):
                            # O modal é aberto diretamente no clique. Não persistimos
                            # o alvo em session_state, pois isso fazia o modal reaparecer
                            # em todo rerun da aplicação (ex.: alteração do termo inicial).
                            _modal_evidencia_cnis(cnpj, ev0, img)
                    with b2:
                ...
                with c3:
                    ...
                    # A chave incorpora a classificação automática/origem.
                    # Assim, uma nova decisão do CNIS não herda o estado visual
                    # antigo do widget (por exemplo, Progressiva).
                    widget_state_id = f"{current}_{'manual' if manual_confirmada else 'automatico'}"
                    grp_key = 'grp_' + re.sub(r'\W+', '_', cnpj) + '_' + re.sub(r'\W+', '_', widget_state_id)
                    val = st.selectbox(
                ...
                # IMPORTANTE: não persistir o valor do selectbox aqui.
                # Alteração no widget sem clique em CONFIRMAR é apenas uma edição
                # pendente. A persistência manual ocorre exclusivamente no callback.
                if not manual_confirmada and cnpj not in st.session_state.classification_meta:
                    st.session_state.classification_meta[cnpj] = {
                        **meta,
                        'classificacao_final': current,
                        'origem_classificacao': meta.get('origem_classificacao', 'automatico'),
                        'confirmacao_manual_explicita': False,
                    }
                ...
                persisted = final_meta.get('classificacao_final', current)
                if val != persisted:
                    st.info(f'✏️ Alteração pendente: {_classificacao_label(val, meta)}. Clique em **CONFIRMAR CLASSIFICAÇÃO** para aplicar esta escolha ao motor.')
                elif persisted == 'nao_definido':
                    st.warning('⚠️ Classificação não definida. A Verificação não realizará classificação previdenciária silenciosa para esta fonte.')
                elif final_meta.get('confirmacao_manual_explicita'):
                    st.success(f'✓ Classificação confirmada pelo usuário: {_classificacao_label(persisted, meta)}.')
                elif meta['nivel_confianca'] == 'alta':
                    st.success(f'✓ Classificação automática aplicada: {_classificacao_label(persisted, meta)}.')
                else:
                    st.warning(f'⚠️ Sugestão {_classificacao_label(persisted, meta)} com confiança {conf_label.get(meta["nivel_confianca"], meta["nivel_confianca"])}. Recomenda-se confirmar.')
